WITH
     ...
     filterBadAdress AS (SELECT (SELECT master.dbo.FracToDec(andel)) 'fra',FNR,BETECKNING,ärndenr 'arndenr',Namn,Adress,POSTNUMMER,postOrt,PERSORGNR FROM tempExcel.dbo.InputPlusGeofir),

     X as (SELECT fra,POSTORT,POSTNUMMER,ADRESS,NAMN,BETECKNING,arndenr,PERSORGNR,RowNum FROM (SELECT q.fra,q.POSTORT,q.POSTNUMMER,q.ADRESS,q.NAMN,q.BETECKNING,q.arndenr,q.PERSORGNR,ROW_NUMBER() OVER (PARTITION BY q.arndenr ORDER BY q.fra DESC) RowNum FROM filterBadAdress AS q INNER JOIN filterBadAdress thethree ON q.arndenr = thethree.arndenr AND q.namn = thethree.namn) as Xtemp WHERE Xtemp.postOrt <> '' AND Xtemp.POSTNUMMER <> '' AND Xtemp.Adress <> '' AND Xtemp.Namn IS NOT NULL),


     filterSmallOwnersBadAdress AS (select * from X where  X.RowNum = 1
                                    UNION
                                    select * from  X where X.RowNum > 1 AND X.RowNum < 4 AND fra > 0.3 ),
